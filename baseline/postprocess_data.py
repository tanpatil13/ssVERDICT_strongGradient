...
def plot_param_maps(f_ic_map, f_ees_map, d_ees_map, r_map, f_vasc_map, cell_map, zslice, th_gradient_strength, timestamp, target_dir, control_type, control_id=None, cmap='jet'):
    ...
    fig.savefig(target_dir + '/model_output_directory/' + timestamp + f'/baseline_param_maps_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.png', dpi=300, bbox_inches='tight')

    ficsave = nib.Nifti1Image(f_ic_map, np.eye(4))
    nib.save(ficsave, target_dir + '/model_output_directory/' + timestamp + f'/baseline_f_ic_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    feessave = nib.Nifti1Image(f_ees_map, np.eye(4))
    nib.save(feessave, target_dir + '/model_output_directory/' + timestamp + f'/baseline_f_ees_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    fvascsave = nib.Nifti1Image(f_vasc_map, np.eye(4))
    nib.save(fvascsave, target_dir + '/model_output_directory/' + timestamp + f'/baseline_f_vasc_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    deessave = nib.Nifti1Image(d_ees_map, np.eye(4))
    nib.save(deessave, target_dir + '/model_output_directory/' + timestamp + f'/baseline_d_ees_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    rsave = nib.Nifti1Image(r_map, np.eye(4))
    nib.save(rsave, target_dir + '/model_output_directory/' + timestamp + f'/baseline_r_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')
...
def generate_param_maps(f_ic_pred, f_ees_pred, d_ees_pred, r_pred, image_mask, th_gradient_strength, timestamp, target_dir, zslice, control_type, control_id=None, prostate_mask_dir=None, prostate_mask_file_pattern=None):
    ...
    return f_ic_map, f_ees_map, d_ees_map, f_vasc_map, r_map, cell_map