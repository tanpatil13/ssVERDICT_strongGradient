...
def plot_param_maps(MD_map, FA_map, MK_map, AK_map, RK_map, zslice, timestamp, control_type):
    """
    ...
    MD_plot = ax[0].imshow(MD_map[:, :, zslice], cmap='jet') 
    ...
    FA_plot = ax[1].imshow(FA_map[:, :, zslice], cmap='jet')
    ...
    fig.savefig(timestamp + f'/ssDKI_3D_param_maps_{control_type}_{timestamp}.png', dpi=300, bbox_inches='tight')

    MDsave = nib.Nifti1Image(MD_map, np.eye(4))
    nib.save(MDsave, timestamp + f'/ssDKI_3D_MD_{control_type}_{timestamp}.nii.gz')

    FAsave = nib.Nifti1Image(FA_map, np.eye(4))
    nib.save(FAsave, timestamp + f'/ssDKI_3D_FA_{control_type}_{timestamp}.nii.gz')

    MKsave = nib.Nifti1Image(MK_map, np.eye(4))
    nib.save(MKsave, timestamp + f'/ssDKI_3D_MK_{control_type}_{timestamp}.nii.gz')

    AKsave = nib.Nifti1Image(AK_map, np.eye(4))
    nib.save(AKsave, timestamp + f'/ssDKI_3D_AK_{control_type}_{timestamp}.nii.gz')

    RKsave = nib.Nifti1Image(RK_map, np.eye(4))
    nib.save(RKsave, timestamp + f'/ssDKI_3D_RK_{control_type}_{timestamp}.nii.gz')


def generate_param_maps(MD_pred, FA_pred, MK_pred, AK_pred, RK_pred, image_mask, timestamp, zslice, control_type):
    """
    ...
    plot_param_maps(MD_map, FA_map, MK_map, AK_map, RK_map, zslice, timestamp, control_type)
