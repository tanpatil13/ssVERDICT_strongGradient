...
def train_model(model, train_dataloader, val_dataloader, criterion, optmizier, num_epochs='30', device='cuda', timestamp="", scheduler=None):
    ...
    for epoch in range(num_epochs):
        ...
        epoch_train_loss, train_D_k_pred, train_K_pred = train_single_epoch(model, train_dataloader, criterion, optmizier, device)
...
def plot_loss_curves(train_losses, val_losses, timestamp):
    ...
    plt.savefig(timestamp + f'/ssDKI_1D_train_val_loss_curves_{timestamp}.png', dpi=300, bbox_inches='tight')
...
def perform_training_inference(grad_dataset_dir, train_data_dir, val_data_dir, healthy_test_data_dir, patient_test_data_dir,
    ...
    model = ssDKI_NN(th_bvals_all_grad, nparams, device).to(device)
    ...
    healthy_test_D_k_map, healthy_test_K_map = generate_param_maps(healthy_test_D_k_pred, healthy_test_K_pred, healthy_test_image_G300_mask, timestamp, 7, "healthy")
    ...
    patient_test_D_k_map, patient_test_K_map = generate_param_maps(patient_test_D_k_pred, patient_test_K_pred, patient_test_image_G300_mask, timestamp, 8, "patient")