...
def plot_param_maps(f_ic_map, f_ees_map, r_map, f_vasc_map, cell_map, zslice, timestamp, control_type):
    ...
    fig.savefig(timestamp + f'/ssVERDICT_NN_param_maps_{control_type}_{timestamp}.png', dpi=300, bbox_inches='tight')

    ficsave = nib.Nifti1Image(f_ic_map, np.eye(4))
    nib.save(ficsave, timestamp + f'/ssVERDICT_NN_f_ic_{control_type}_{timestamp}.nii.gz')

    feessave = nib.Nifti1Image(f_ees_map, np.eye(4))
    nib.save(feessave, timestamp + f'/ssVERDICT_NN_f_ees_{control_type}_{timestamp}.nii.gz')

    fvascsave = nib.Nifti1Image(f_vasc_map, np.eye(4))
    nib.save(fvascsave, timestamp + f'/ssVERDICT_NN_f_vasc_{control_type}_{timestamp}.nii.gz')

    rsave = nib.Nifti1Image(r_map, np.eye(4))
    nib.save(rsave, timestamp + f'/ssVERDICT_NN_r_{control_type}_{timestamp}.nii.gz')
...
def generate_param_maps(f_ic_pred, f_ees_pred, r_pred, image_mask, timestamp, zslice, control_type):
    ...
    plot_param_maps(f_ic_map, f_ees_map, r_map, f_vasc_map, cell_map, zslice, timestamp, control_type)
