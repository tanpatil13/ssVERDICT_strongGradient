...
def plot_param_maps(D_k_map, K_map, zslice, th_gradient_strength, timestamp, target_dir, control_type, control_id=None, cmap='jet'):
    ...
    fig.savefig(target_dir + '/model_output_directory/' + timestamp + f'/ssDKI_1D_denseMLP_param_maps_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.png', dpi=300, bbox_inches='tight')

    Dksave = nib.Nifti1Image(D_k_map, np.eye(4))
    nib.save(Dksave, target_dir + '/model_output_directory/' + timestamp + f'/ssDKI_1D_denseMLP_D_k_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    Ksave = nib.Nifti1Image(K_map, np.eye(4))
    nib.save(Ksave, target_dir + '/model_output_directory/' + timestamp + f'/ssDKI_1D_denseMLP_K_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')
