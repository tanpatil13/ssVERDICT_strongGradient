...
def plot_param_maps(D_k_map, K_map, zslice, th_gradient_strength, timestamp, target_dir, control_type, control_id=None, cmap='jet'):
    """
    Plots the parameter maps for D_k, K.
    ...
    fig.savefig(target_dir + '/model_output_directory/' + timestamp + f'/DKI_1D_NLLS_param_maps_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.png', dpi=300, bbox_inches='tight')

    Dksave = nib.Nifti1Image(D_k_map, np.eye(4))
    nib.save(Dksave, target_dir + '/model_output_directory/' + timestamp + f'/DKI_1D_NLLS_D_k_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    Ksave = nib.Nifti1Image(K_map, np.eye(4))
    nib.save(Ksave, target_dir + '/model_output_directory/' + timestamp + f'/DKI_1D_NLLS_K_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')
...
def generate_param_maps(D_k_pred, K_pred, image_mask, th_gradient_strength, timestamp, target_dir, zslice, control_type, control_id=None, prostate_mask_dir=None, prostate_mask_file_pattern=None):
    """
    Generates parameter voxel array from the predicted values of D_k and K by normalizing and constraining them.
