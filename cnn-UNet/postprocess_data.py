...
def plot_param_maps(f_ic_map, f_ees_map, d_ees_map, r_map, f_vasc_map, cellularity_map, zslice, th_gradient_strength, timestamp, target_dir, control_type, control_id=None, cmap='jet'):
    """
    ...
    fig.savefig(target_dir + '/model_output_directory/' + timestamp + f'/ssVERDICT_UNet_param_maps_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.png', dpi=300, bbox_inches='tight')

    ficsave = nib.Nifti1Image(f_ic_map, np.eye(4))
    nib.save(ficsave, target_dir + '/model_output_directory/' + timestamp + f'/ssVERDICT_UNet_f_ic_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    feessave = nib.Nifti1Image(f_ees_map, np.eye(4))
    nib.save(feessave, target_dir + '/model_output_directory/' + timestamp + f'/ssVERDICT_UNet_f_ees_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    fvascsave = nib.Nifti1Image(f_vasc_map, np.eye(4))
    nib.save(fvascsave, target_dir + '/model_output_directory/' + timestamp + f'/ssVERDICT_UNet_f_vasc_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    deessave = nib.Nifti1Image(d_ees_map, np.eye(4))
    nib.save(deessave, target_dir + '/model_output_directory/' + timestamp + f'/ssVERDICT_UNet_d_ees_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    rsave = nib.Nifti1Image(r_map, np.eye(4))
    nib.save(rsave, target_dir + '/model_output_directory/' + timestamp + f'/ssVERDICT_UNet_r_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')
...
def generate_param_maps(f_ic_pred, f_ees_pred, d_ees_pred, r_pred, image_mask, th_gradient_strength, timestamp, target_dir, zslice, control_type, control_id=None, prostate_mask_dir=None, prostate_mask_file_pattern=None):
    ...
    f_ic = f_ic.transpose(1, 2, 0)
    f_ees = f_ees.transpose(1, 2, 0)
    d_ees = d_ees.transpose(1, 2, 0)
    r = r.transpose(1, 2, 0)
    ...
    A = f_vasc
    normA = A - A.min()
    f_vasc = 0.2 * (normA/(normA.max()))       # constraining fvasc
    f_ees = f_ees/(f_ic + f_ees + f_vasc)
    f_ic = f_ic/(f_ic + f_ees + f_vasc)
    cellularity = f_ic/r**3

    f_ic_map, f_ees_map, f_vasc_map, d_ees_map, r_map, cell_map = f_ic, f_ees, f_vasc, d_ees, r, cellularity
    ...
    if prostate_mask_file_pattern and prostate_mask_dir:
        ...
        prostate_mask = np.repeat(prostate_mask[:, :, np.newaxis], 14, axis=2)
