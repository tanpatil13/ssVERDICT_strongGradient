...
from commons.preprocess_data_cnn import preprocess_images
from autoencoder_model import ssVERDICT_UNet
...
class StrongGradientDataset(torch.utils.data.Dataset):
    ...
    Returns image data and corresponding prostate mask.
    """

    def __init__(self, image_data, prostate_mask):
    ...
    def __getitem__(self, idx):
        ...
        mask = torch.FloatTensor(self.prostate_mask[idx])
        return image, mask
...
def create_single_dataloader(preprocessed_image_data, batch_size=256, shuffle=False):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset=preprocessed_image_data,
...
def train_single_epoch(model, train_dataloader, criterion, optimizer, device='cuda'):
    ...
    for S_train, mask_train in tqdm(train_dataloader, desc="Training", unit="batch"):
        S_train = S_train.to(device)
        mask_train = mask_train.to(device)

        S_pred, f_ic_pred, f_ees_pred, d_ees_pred, r_pred = model(S_train)
        
        mask_train = mask_train.expand(-1, S_train.shape[1], -1, -1)
        S_pred_masked = S_pred * mask_train
        S_train_masked = S_train * mask_train

        loss_map = criterion(S_pred_masked, S_train_masked)
        mask_train = mask_train.float()
        loss_masked = loss_map * mask_train
        loss_per_sample = loss_masked.sum(dim=(1,2,3)) / mask_train.sum(dim=(1,2,3))
        ...
        all_f_ic_pred.append(f_ic_pred.detach().cpu().numpy())
        all_f_ees_pred.append(f_ees_pred.detach().cpu().numpy())
        all_d_ees_pred.append(d_ees_pred.detach().cpu().numpy())
        all_r_pred.append(r_pred.detach().cpu().numpy())
    
    all_f_ic_pred = np.concatenate(all_f_ic_pred, axis=0)
    all_f_ees_pred = np.concatenate(all_f_ees_pred, axis=0)
    all_d_ees_pred = np.concatenate(all_d_ees_pred, axis=0)
...
def val_test_model(split_type, model, val_test_dataloader, criterion, device='cuda'):
    ...
    with torch.no_grad():
        for S_val_test, mask_val_test in tqdm(val_test_dataloader, desc=f"{split_type} Evaluation", unit="batch"):
            S_val_test = S_val_test.to(device)
            mask_val_test = mask_val_test.to(device)

            S_pred, f_ic_pred, f_ees_pred, d_ees_pred, r_pred = model(S_val_test)

            mask_val_test = mask_val_test.expand(-1, S_val_test.shape[1], -1, -1)
            S_pred_masked = S_pred * mask_val_test
            S_val_test_masked = S_val_test * mask_val_test

            loss_map = criterion(S_pred_masked, S_val_test_masked)
            mask_val_test = mask_val_test.float()
            loss_masked = loss_map * mask_val_test
            loss_per_sample = loss_masked.sum(dim=(1,2,3)) / mask_val_test.sum(dim=(1,2,3))
            loss = loss_per_sample.mean()
...
def perform_training_inference(grad_dataset_dir, train_data_dir, val_data_dir, healthy_test_data_dir, patient_test_data_dir,
    ...
    _, train_preprocessed_image_data, _, train_prostate_masks = preprocess_images(train_data_dir,
    ...
    _, val_preprocessed_image_data, _, val_prostate_masks = preprocess_images(val_data_dir,
                                                                grad_dataset_dir, image_file_pattern, x_bvec_file_pattern, 
                                                                y_bvec_file_pattern, z_bvec_file_pattern, prostate_mask_file_pattern, th_bvals)

    _, healthy_test_preprocessed_image_data, _, healthy_test_prostate_mask = preprocess_images(healthy_test_data_dir,
                                                                grad_dataset_dir, image_file_pattern, x_bvec_file_pattern, 
                                                                y_bvec_file_pattern, z_bvec_file_pattern, prostate_mask_file_pattern, th_bvals)

    _, patient_test_preprocessed_image_data, _, patient_test_prostate_mask = preprocess_images(patient_test_data_dir,
                                                                grad_dataset_dir, image_file_pattern, x_bvec_file_pattern, 
                                                                y_bvec_file_pattern, z_bvec_file_pattern, prostate_mask_file_pattern, th_bvals)
    ...
    train_dataset = StrongGradientDataset(train_preprocessed_image_data, train_prostate_masks)
    val_dataset = StrongGradientDataset(val_preprocessed_image_data, val_prostate_masks)
    healthy_test_dataset = StrongGradientDataset(healthy_test_preprocessed_image_data, healthy_test_prostate_mask)
    patient_test_dataset = StrongGradientDataset(patient_test_preprocessed_image_data, patient_test_prostate_mask)
    ...
    model = ssVERDICT_UNet(th_bvals_scaled, Delta.to(device), delta.to(device), gradient_strength.to(device), nparams, device, batch_size).to(device)
    ...
    print("Model Summary: ")
    summary(model, input_size=(batch_size, train_preprocessed_image_data.shape[1], train_preprocessed_image_data.shape[2], train_preprocessed_image_data.shape[3]))
    ...
    healthy_test_f_ic_map, healthy_test_f_ees_map, healthy_test_f_vasc_map, healthy_test_d_ees_map, healthy_test_r_map, _ \
        = generate_param_maps(healthy_test_f_ic_pred, healthy_test_f_ees_pred, healthy_test_d_ees_pred, healthy_test_r_pred, healthy_test_prostate_mask, th_gradient_strength, timestamp, target_dir, 7, "healthy", "4", healthy_test_prostate_mask_path, prostate_mask_file_pattern)
    ...
    patient_test_f_ic_map, patient_test_f_ees_map, patient_test_f_vasc_map, patient_test_d_ees_map, patient_test_r_map, _ \
        = generate_param_maps(patient_test_f_ic_pred, patient_test_f_ees_pred, patient_test_d_ees_pred, patient_test_r_pred, patient_test_prostate_mask, th_gradient_strength, timestamp, target_dir, 8, "patient", "4", patient_test_prostate_mask_path, prostate_mask_file_pattern)

    print("Inference on test set completed.")

    print("Beginning inference on individual patients...")

    _, patient_1_preprocessed_image_data, _, patient_1_prostate_mask = preprocess_images(patient_1_data_dir,
                                                                grad_dataset_dir, image_file_pattern, x_bvec_file_pattern, 
                                                                y_bvec_file_pattern, z_bvec_file_pattern, prostate_mask_file_pattern, th_bvals)
    _, patient_2_preprocessed_image_data, _, patient_2_prostate_mask = preprocess_images(patient_2_data_dir,
                                                                grad_dataset_dir, image_file_pattern, x_bvec_file_pattern, 
                                                                y_bvec_file_pattern, z_bvec_file_pattern, prostate_mask_file_pattern, th_bvals)
    _, patient_3_preprocessed_image_data, _, patient_3_prostate_mask = preprocess_images(patient_3_data_dir,
                                                                grad_dataset_dir, image_file_pattern, x_bvec_file_pattern, 
                                                                y_bvec_file_pattern, z_bvec_file_pattern, prostate_mask_file_pattern, th_bvals)
    _, patient_5_preprocessed_image_data, _, patient_5_prostate_mask = preprocess_images(patient_5_data_dir,
                                                                grad_dataset_dir, image_file_pattern, x_bvec_file_pattern, 
                                                                y_bvec_file_pattern, z_bvec_file_pattern, prostate_mask_file_pattern, th_bvals)

    patient_1_test_dataset = StrongGradientDataset(patient_1_preprocessed_image_data, patient_1_prostate_mask)
    patient_2_test_dataset = StrongGradientDataset(patient_2_preprocessed_image_data, patient_2_prostate_mask)
    patient_3_test_dataset = StrongGradientDataset(patient_3_preprocessed_image_data, patient_3_prostate_mask)
    patient_5_test_dataset = StrongGradientDataset(patient_5_preprocessed_image_data, patient_5_prostate_mask)

    patient_1_dataloader = create_single_dataloader(patient_1_test_dataset, batch_size)
    patient_2_dataloader = create_single_dataloader(patient_2_test_dataset, batch_size)
    patient_3_dataloader = create_single_dataloader(patient_3_test_dataset, batch_size)
    patient_5_dataloader = create_single_dataloader(patient_5_test_dataset, batch_size)
    ...
    patient_1_f_ic_map, patient_1_f_ees_map, patient_1_f_vasc_map, patient_1_d_ees_map, patient_1_r_map, _ \
        = generate_param_maps(patient_1_test_f_ic_pred, patient_1_test_f_ees_pred, patient_1_test_d_ees_pred, patient_1_test_r_pred, patient_1_prostate_mask, th_gradient_strength, timestamp, target_dir, 5, "patient", "1", patient_1_prostate_mask_path, prostate_mask_file_pattern)
    ...
    patient_2_f_ic_map, patient_2_f_ees_map, patient_2_f_vasc_map, patient_2_d_ees_map, patient_2_r_map, _ \
        = generate_param_maps(patient_2_test_f_ic_pred, patient_2_test_f_ees_pred, patient_2_test_d_ees_pred, patient_2_test_r_pred, patient_2_prostate_mask, th_gradient_strength, timestamp, target_dir, 7, "patient", "2", patient_2_prostate_mask_path, prostate_mask_file_pattern)
    ...
    patient_3_f_ic_map, patient_3_f_ees_map, patient_3_f_vasc_map, patient_3_d_ees_map, patient_3_r_map, _ \
        = generate_param_maps(patient_3_test_f_ic_pred, patient_3_test_f_ees_pred, patient_3_test_d_ees_pred, patient_3_test_r_pred, patient_3_prostate_mask, th_gradient_strength, timestamp, target_dir, 6, "patient", "3", patient_3_prostate_mask_path, prostate_mask_file_pattern)
    ...
    patient_5_f_ic_map, patient_5_f_ees_map, patient_5_f_vasc_map, patient_5_d_ees_map, patient_5_r_map, _ \
        = generate_param_maps(patient_5_test_f_ic_pred, patient_5_test_f_ees_pred, patient_5_test_d_ees_pred, patient_5_test_r_pred, patient_5_prostate_mask, th_gradient_strength, timestamp, target_dir, 7, "patient", "5", patient_5_prostate_mask_path, prostate_mask_file_pattern)
