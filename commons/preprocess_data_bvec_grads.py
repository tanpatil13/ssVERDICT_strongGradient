...
def preprocess_images(split_data_dir, grad_dataset_dir,
    ...
    preprocessed_x_bvec_data = []
    preprocessed_y_bvec_data = []
    preprocessed_z_bvec_data = []
    ...
    for subject_type, subject_ids in split_data_dir.items():
        for subject_id in subject_ids:
            ...
            norm_x_bvec_data = x_bvec_data / (ac_bvals + 1e-9)
            ...
            norm_avg_x_bvec_data = np.zeros((x_bvec_data_flattened_masked.shape[0], len(th_bvals)))
            norm_avg_y_bvec_data = np.zeros((y_bvec_data_flattened_masked.shape[0], len(th_bvals)))
            norm_avg_z_bvec_data = np.zeros((z_bvec_data_flattened_masked.shape[0], len(th_bvals)))

            for i in range(len(th_bvals)):
                avg_image_data[:, i] = np.mean(image_data_flattened_masked[:, ac_th_bvals_map == i], axis=1)
                norm_avg_y_bvec_data[:, i] = np.mean(y_bvec_data_flattened_masked[:, ac_th_bvals_map == i], axis=1)
            ...
            preprocessed_x_bvec_data.extend(norm_avg_x_bvec_data)
