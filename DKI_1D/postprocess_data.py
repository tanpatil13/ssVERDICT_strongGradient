...
def plot_param_maps(D_k_map, K_map, zslice, th_gradient_strength, timestamp, control_type, control_id=None, cmap='jet'):
    """
    ...
    fig.savefig(timestamp + f'/ssDKI_1D_param_maps_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.png', dpi=300, bbox_inches='tight')

    Dksave = nib.Nifti1Image(D_k_map, np.eye(4))
    nib.save(Dksave, timestamp + f'/ssDKI_1D_D_k_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    Ksave = nib.Nifti1Image(K_map, np.eye(4))
    nib.save(Ksave, timestamp + f'/ssDKI_1D_K_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

def generate_param_maps(D_k_pred, K_pred, image_mask, th_gradient_strength, timestamp, zslice, control_type, control_id=None, prostate_mask_dir=None, prostate_mask_file_pattern=None):
    """
    ...
    plot_param_maps(D_k_map, K_map, zslice, th_gradient_strength, timestamp, control_type, control_id, cmap)
