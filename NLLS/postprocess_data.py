...
def plot_param_maps(f_ic_map, f_ees_map, r_map, f_vasc_map, cell_map, zslice, th_gradient_strength, timestamp, target_dir, control_type, control_id=None, cmap='jet'):
    ...
    fig.savefig(target_dir + '/model_output_directory/' + timestamp + f'/VERDICT_NLLS_param_maps_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.png', dpi=300, bbox_inches='tight')

    ficsave = nib.Nifti1Image(f_ic_map, np.eye(4))
    nib.save(ficsave, target_dir + '/model_output_directory/' + timestamp + f'/VERDICT_NLLS_f_ic_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')

    feessave = nib.Nifti1Image(f_ees_map, np.eye(4))
    nib.save(feessave, target_dir + '/model_output_directory/' + timestamp + f'/VERDICT_NLLS_f_ees_{control_type}_{control_id}_{th_gradient_strength}_{timestamp}.nii.gz')
