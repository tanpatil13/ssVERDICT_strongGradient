...
def plot_param_maps(f_ic_map, f_ees_map, r_map, f_vasc_map, cell_map, zslice, timestamp, control_type):
    ...
    fig.savefig(timestamp + f'/VERDICT_NLLS_param_maps_{control_type}_{timestamp}.png', dpi=300, bbox_inches='tight')

    ficsave = nib.Nifti1Image(f_ic_map, np.eye(4))
    nib.save(ficsave, timestamp + f'/VERDICT_NLLS_f_ic_{control_type}_{timestamp}.nii.gz')

    feessave = nib.Nifti1Image(f_ees_map, np.eye(4))
    nib.save(feessave, timestamp + f'/VERDICT_NLLS_f_ees_{control_type}_{timestamp}.nii.gz')

    fvascsave = nib.Nifti1Image(f_vasc_map, np.eye(4))
    nib.save(fvascsave, timestamp + f'/VERDICT_NLLS_f_vasc_{control_type}_{timestamp}.nii.gz')

    rsave = nib.Nifti1Image(r_map, np.eye(4))
    nib.save(rsave, timestamp + f'/VERDICT_NLLS_r_{control_type}_{timestamp}.nii.gz')
